...
determine_gifts.out: curl.out xmlparse.out main.out
	$(COMPILER) -o $@ $^ $(LINK_LIBCURL_FLAGS) $(LINK_XML_FLAGS)

determine_gifts_debug.out: curl_debug.out xmlparse_debug.out main_debug.out
	$(COMPILER) -o $@ $^ $(LINK_LIBCURL_FLAGS) $(LINK_XML_FLAGS)
...
xmlparse.out: xmlparse.cpp
	$(COMPILER) $(ERRORS_FLAGS) $(WARNINGS_FLAGS) $(LEAK_FLAGS) -c $^ -o $@ $(LINK_XML_FLAGS)
